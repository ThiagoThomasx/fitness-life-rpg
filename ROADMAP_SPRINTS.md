...
- [x] Parte 2 — `buildHealthRecoveryDashboard` (agregador puro de página); nova rota `/saude` (fora da navegação principal, acessível via card em Configurações); resumo, sono/FC de repouso/passos/atividade (componente único reutilizado, gráfico + baseline + tendência), peso (leitura de Body Progress), qualidade (sem score único), conflitos (sem resolução automática); 6 testes novos, 1414/1414 no total; lint/typecheck/build limpos; QA manual real sem erro de console, sem overflow em 375px
